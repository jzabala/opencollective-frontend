...
const getItemInfo = (item, isInverted) => {
  switch (item.__typename) {
    ...
    case 'Order':
      return {
        isCredit: item.type === TransactionTypes.CREDIT ? !isInverted : isInverted,
        amount: item.amount,
        collective: item.fromCollective,
        paymentMethod: item.paymentMethod,
        transaction: item,
 ...
 * A single item
 */
const BudgetItem = ({ item, isInverted, isCompact, intl }) => {
  ...
  const isRefund = Boolean(transaction && transaction.refundTransaction);
  const hasInvoiceBtn = !isExpense && !isRefund && (!isCredit || !isInverted);
 ...
 */
const BudgetItemsList = ({ items, isInverted, isCompact, intl }) => {
